...
% @param max_round: for OLAMA only
% @return: generated schedule
function [schedules, conflict_matrix] = simpleLAMA(slot_len, max_round, is_priority_randomized, is_optimized, row_cnt, column_cnt, interference_range)
    total_node_cnt = row_cnt * column_cnt;
    schedules = cell(total_node_cnt, 1);
    conflict_matrix = repmat(false, total_node_cnt);
