...
// z = x + y; all scaled in dBm
// also called from IMACForwarderPUtils$checkConcurrency()
inline command int16_t SignalMap.dbmSumU(int16_t x, int16_t y) {
	int16_t n, delta, tmp;
...
dbm_t dbmSumS(dbm_t x, dbm_t y) {
	...
	if (x.sign == y.sign) {
		z.sign = x.sign;
		z.abs = call SignalMap.dbmSumU(x.abs, y.abs);
	} else {
