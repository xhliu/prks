...
//	#warning: debug overflow
//	if (deltaI_dB < -32768L || deltaI_dB >= 32768L)
//		assert(deltaI_dB);
	return deltaI_dB;
}
...
//	er_border_gain = (le->rx_er_border_idx != EMPTY_ER_IDX) ? signalMap[le->rx_er_border_idx].inbound_gain : INVALID_GAIN;
	if (call SignalMap.getLocalGain(nb, &in_gain, &out_gain) != SUCCESS)
	...
	call UartLog.logTxRx(DBG_FLAG, DBG_CONTROLLER_FLAG, __LINE__, nb, link_pdr, link_pdr_sample, le->rx_er_border_idx + 1, in_gain / 128, -le->rx_nI.abs / 128); // call SignalMap.getSignalMapSize(), reference_pdr);
