...
//		call BusyWait.wait(MIN_CW + backoff);
	...
	if (!isScheduled) {
		if (slot_since_tdma_start < (uint32_t)active_link_size * FRAME_LEN) {
			...
			// this is right bcoz ftsp slot in the last one in a frame
			...
			// SCREAM slot
				...
				if (round_offset < (ROUND_LEN - 1)) {
					...
					if (is_any_tx_fail) {
						...
						//if (is_any_tx_fail)
						//call UartLog.logEntry(DBG_FLAG, DBG_TDMA_FLAG, __LINE__, is_any_tx_fail);
					}
				} else {
				// make decision & ftsp
				// TODO: move to the 1st slot of next round to save 1 slot
					...
					if (my_outgoing_link_idx == active_link_idx) {
						...
						if (is_any_tx_fail)
							call UartLog.logEntry(DBG_FLAG, DBG_TDMA_FLAG, __LINE__, round_offset);
					}
...
//	}
	...
	//if (!is_ftsp_slot) {
	if (beacon_cnt >= COMM_SUBSLOT_FTSP_BEACON_CNT) {
		uint8_t my_link_set_idx;
		uint32_t current_slot = g_next_firing_time / SLOT_LEN - 1;
		//call CC2420Packet.setPower(m_control_p, CONTROL_POWER_LEVEL);
		call PacketTransmitPower.set(m_control_p, CONTROL_POWER_LEVEL);
	#warning TDMA not CSMA
		//my_link_set_idx = call Util.findLinkIdx(my_ll_addr, call Util.getReceiver());
		//if (my_link_set_idx != beacon_cnt - COMM_SUBSLOT_FTSP_BEACON_CNT)
		//	return;
		my_link_set_idx = beacon_cnt - COMM_SUBSLOT_FTSP_BEACON_CNT + 1 + (current_slot & 0x7) * COMM_SUBSLOT_CTRL_BEACON_CNT;
		if (TOS_NODE_ID != my_link_set_idx)
			return;
		ret = call CtrlSend.send(AM_BROADCAST_ADDR, m_control_p, 0);
		if (SUCCESS == ret) {
			elapsed_slot_since_last_win = 0;
		}
		call UartLog.logTxRx(DBG_FLAG, DBG_TDMA_FLAG, __LINE__, 0, 0, my_link_set_idx, beacon_cnt, ret, current_slot);
	} else {
...
void txrxCtrl(uint32_t current_slot) {
#ifdef SCREAM
	uint16_t backoff;
	backoff = call Random.rand16();
	backoff &= CW_HEX_MODULAR;
