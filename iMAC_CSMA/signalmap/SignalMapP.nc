...
uint8_t powerControl(uint8_t prev_tx_power_level, int16_t outbound_gain, int16_t nb_i) {
	uint8_t level;
	int16_t tx_signal, snr;
	
	if (prev_tx_power_level != INVALID_POWER_LEVEL) {
		snr = level2Power(prev_tx_power_level) - outbound_gain - nb_i;
		if (snr < GAMMA / DELTA) {
			tx_signal = nb_i + GAMMA + outbound_gain;
			level = power2LevelFloor(tx_signal);
		} else if (snr > GAMMA * DELTA) {
			tx_signal = nb_i + GAMMA + outbound_gain;
			level = power2LevelCeil(tx_signal);
		} else {
			level = prev_tx_power_level;
		}
	} else {
		tx_signal = nb_i + GAMMA + outbound_gain;
		level = power2LevelFloor(tx_signal);
...
command error_t SignalMap.getInterferenceThresholdPowerLevelNI(am_addr_t nb, bool is_sender, int16_t *interference_threshold, int16_t *min_interfererence_threshold, uint8_t *power_level, int16_t *node_i_) {
	...
	sm_entry_t *se, *nb_se;
	int16_t outbound_gain, nb_i, min_threshold, rx_i;
	int16_t max_outbound_gain = 0;
	int16_t max_nb_i = 0;
	...
	se = &signalMap[idx];
	// store to update prev_tx_power_level computed later
	nb_se = se;
	...
	*node_i_ = (node_I.abs >> SCALE_L_SHIFT_BIT);
	...
	//if (!is_sender)
		//call UartLog.logTxRx(DBG_FLAG, DBG_ER_FLAG, 5, 0, idx, nb, power_level_reuse_cnt, prev_max_er_border_idx, er_border_idx);
	// tx power
	...
	if (se->node_i != INVALID_DBM && se->outbound_gain != INVALID_GAIN) {
		...
		//max_tx_signal = outbound_gain + nb_i + GAMMA;
		//level = power2LevelCeil(outbound_gain + nb_i + GAMMA);
		level = powerControl(se->prev_tx_power_level, outbound_gain, nb_i);
		if (max_level < level) {
			max_level = level;
			max_outbound_gain = outbound_gain;
			max_nb_i = nb_i;
		}
	} else {
	...
	*min_interfererence_threshold = min_threshold;
	...
	// use power level no smaller than data; otherwise cannot reach other nodes
	max_level = (max_level > CC2420_DEF_RFPOWER) ? max_level : CC2420_DEF_RFPOWER;
	nb_se->prev_tx_power_level = max_level;
	// TODO: debug
	//max_level = 31;
	*power_level = max_level;
	call LinkEstimator.getInDataPdr(nb, &link_pdr, &link_pdr_sample);
	if (!is_sender)
		call UartLog.logTxRx(DBG_FLAG, DBG_ER_FLAG, 55, nb, -rx_i, max_level, -*node_i_, er_border_idx + 1, link_pdr);
		//call UartLog.logTxRx(DBG_FLAG, DBG_ER_FLAG, 55, nb, -rx_i, max_level, -max_nb_i, er_border_idx + 1, max_outbound_gain);
	return SUCCESS;
}
...
error_t updateER(int16_t idx, bool is_tx, dbm_t delta_I_d) {
	...
	if (delta_I_d.sign < 0) {
		...
		if (-1 == er_border_idx) {
			...
			return assertGain(is_tx, nb_se, se->inbound_gain, idx, OFFSET);
		}
		...
		//call UartLog.logTxRx(DBG_FLAG, DBG_ER_FLAG, 7, nb_se->nb, i, er_border_idx, se->inbound_gain, -total_i / 128, -delta_i_d / 128);
	...
	if (idx < SM_SIZE) {
		...
		if (in_gain != INVALID_GAIN) {
			...
			if (idx < SM_SIZE) {
				...
				se->inbound_gain = in_gain;
				se->outbound_gain = out_gain;
				se->prev_tx_power_level = INVALID_POWER_LEVEL;
